...
import { PrettierError, ReScriptError, ErrorCode } from '../utils/errors.js';

export class PrettierTransformer implements ProcessingStep {
  public readonly name = 'prettier';
  public readonly description = 'Code formatting using Prettier';
  ...
  async execute(input: ProcessingInput): Promise<ProcessingOutput> {
    ...
    try {
      ...
      const prettierError = new PrettierError(
   ...
   */
  private async detectParser(code: string, fileName?: string): Promise<string> {
  ...
  private async formatWithRetry(
    ...
    throw new PrettierError(
      `Failed with all parsers. Last error: ${lastError?.message}`,
      lastError
    );
  }
   ...
   * Calculate formatting metrics
   */
  private calculateMetrics(original: string, formatted: string): {
    originalLines: number;
    formattedLines: number;
    linesDifference: number;
