...
import { z } from 'zod';
import { defaultConfig, validateConfig, validatePartialConfig, mergeConfig, envVarMapping } from './schema.js';
import { InvalidConfigError } from '../utils/errors.js';
const CONFIG_MODULE_NAME = 'rescript';
export class ConfigLoader {
    explorer = cosmiconfigSync(CONFIG_MODULE_NAME, {
        searchPlaces: [
            'package.json',
    ...
    async loadConfig(configPath) {
        ...
        try {
            ...
            throw new InvalidConfigError(`Failed to load config file: ${error instanceof Error ? error.message : String(error)}`, ['Check config file syntax', 'Validate JSON/YAML format']);
        }
    ...
    loadFromEnvironment() {
        ...
        try {
            ...
            throw new InvalidConfigError(`Invalid environment variable configuration: ${error instanceof Error ? error.message : String(error)}`, ['Check environment variable values', 'Ensure correct data types']);
        }
    }
    /**
     * Parse environment variable value to appropriate type
     */
    parseEnvValue(value, configPath) {
        // Boolean values
    ...
    validateCliOverride(options) {
        ...
        if (options.provider || options.model || options.apiKey ||
            ...
            if (typeof options.provider === 'string') {
                ...
                override.provider.name = options.provider;
            }
        ...
        try {
            ...
            throw new InvalidConfigError(`Invalid CLI options: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     * Save configuration to file
     */
    async saveConfig(config, filePath = '.rescriptrc.json') {
        const fs = await import('fs/promises');
        try {
            await fs.writeFile(filePath, JSON.stringify(config, null, 2), 'utf8');
        }
        catch (error) {
            throw new InvalidConfigError(`Failed to save config to ${filePath}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     ...
     */
    validateConfigWithFeedback(config) {
        const result = {
            valid: false,
            config: undefined,
            errors: [],
