...
export class OllamaProvider extends BaseLLMProvider {
    ...
    maxTokens = 32768; // Default context window
    supportsStreaming = true;
    supportsFunctionCalling = false; // Most local models don't support this yet
    baseUrl;
    availableModels = [];
    modelsLoaded = false;
    constructor(config) {
    ...
    async processCode(request) {
        ...
        try {
            ...
            throw new LLMRequestError(this.name, error instanceof Error ? error.message : String(error), this.isNonRetryableError(error), error instanceof Error ? error : undefined);
        }
    }
    /**
     * Create complete prompt for text-based models
     */
    createCompletePrompt(code) {
...
JSON Response:`;
    ...
    async ensureModelAvailable(modelName) {
        ...
        const isAvailable = this.availableModels.some(model => model.name === modelName || model.name.startsWith(modelName.split(':')[0]));
        if (!isAvailable) {
            throw new ReScriptError(ErrorCode.INVALID_MODEL, `Model '${modelName}' not found locally. Available models: ${this.availableModels.map(m => m.name).join(', ')}`, 'ollama-model-check', false, [
    ...
    async loadAvailableModels() {
        try {
            ...
            this.models = this.availableModels.map(model => model.name);
            this.modelsLoaded = true;
            console.log(`📦 Found ${this.availableModels.length} Ollama models`);
        }
        catch (error) {
            throw new ReScriptError(ErrorCode.LLM_REQUEST_FAILED, `Failed to connect to Ollama at ${this.baseUrl}: ${error instanceof Error ? error.message : String(error)}`, 'ollama-connection', false, [
    ...
    async getModelInfo(modelName) {
        ...
        return this.availableModels.find(model => model.name === modelName || model.name.startsWith(modelName.split(':')[0])) || null;
    }
    /**
