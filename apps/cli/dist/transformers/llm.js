...
export class LLMTransformer {
    ...
    async execute(input) {
        ...
        try {
            ...
            const llmError = new LLMRequestError(this.provider.name, error instanceof Error ? error.message : String(error), false, error instanceof Error ? error : undefined);
            return {
                code: input.code,
     ...
     */
    async processSingleChunk(input) {
        ...
        try {
            ...
            const highConfidenceSuggestions = response.suggestions.filter(s => s.confidence >= this.options.minConfidenceThreshold);
            ...
            throw new LLMRequestError(this.provider.name, `Single chunk processing failed: ${error instanceof Error ? error.message : String(error)}`, true, error instanceof Error ? error : undefined);
        }
    }
    /**
     * Process large code by splitting into chunks
     */
    async processInChunks(input) {
     ...
     */
    async applyRenamings(code, suggestions) {
     ...
     */
    async applyRenamingsWithAST(code, suggestions) {
     ...
     */
    async applyRenamingsWithSafeRegex(code, suggestions) {
     ...
     */
    generateWarnings(allSuggestions, appliedSuggestions) {
        const warnings = [];
        const lowConfidenceCount = allSuggestions.length - appliedSuggestions.length;
        if (lowConfidenceCount > 0) {
            warnings.push(`${lowConfidenceCount} suggestions excluded due to low confidence (< ${this.options.minConfidenceThreshold * 100}%)`);
        }
     ...
     */
    static fromConfig(input, options) {
        // Extract provider config from the full config
        const fullConfig = input.config;
        const providerConfig = fullConfig.provider;
        if (!providerConfig) {
            throw new ReScriptError(ErrorCode.INVALID_CONFIG, 'Provider configuration not found in config', 'provider-setup');
        }
